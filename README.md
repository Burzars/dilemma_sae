...
### Расширенный корпус v2 (балансировка под No)

Исходные 8 файлов сильно несбалансированы (~85% Yes, 5% No, 9% «?»), из-за чего
No-направления SAE шумны, а steering не двигает решение. Поэтому корпус
расширяется и из НЕГО делаются два среза.

```bash
export OPENROUTER_API_KEY=sk-...

# 1) Генерация: 10 сценариев × 800 ответов ≈ 8000. Генератор и судья — через
#    OpenRouter. Сначала дёшево проверить пайплайн:
python -m src.generate_answers --smoke
python -m src.generate_answers --out-dir data/results_dilemma_v2

# 2) Снять активации ЛОКАЛЬНО на FULL-срезе (тем же чекпойнтом Qwen2.5-7B-Instruct).

# 3) Два среза из одной генерации:
...
- **Генератор = модель extract.** Тексты генерим `qwen/qwen-2.5-7b-instruct`
  (OpenRouter), активации снимаем `Qwen/Qwen2.5-7B-Instruct` (локально). Это
  ОДНА И ТА ЖЕ модель/версия/chat-template — иначе тексты и активации
  рассогласуются. Зафиксировано в `src/generate_answers.py` (GEN_MODEL) и
  `configs/default.yaml` (model.name).
- **SAE учится на FULL.** Меток для SAE не нужно; больше данных = лучше
  реконструкция, дисбаланс SAE не «переобучает» (он не классификатор).
- **Контраст и выбор триггеров — на BALANCED.** Там метки используются и шум
  редкого класса вреден. Для шага анализа переопределить путь:
  `--override data.results_dir=data/results_dilemma_v2_balanced`.
- Поле `scenario_meta` (sid/risk/benefit/others) едет в оба среза — нужно для
  отбора промптов по податливости в steering. `data.py` его игнорирует.
...
export OPENROUTER_API_KEY=sk-...
python scripts/07_steering.py --smoke   # дёшево — проверка пайплайна
python scripts/07_steering.py           # полный прогон
```

Шаги 2→6→3→4 можно прогнать одной командой (после локальной экстракции на
FULL-срезе) через оркестратор:

```bash
python run_pipeline.py --smoke    # дёшево, сквозная проверка
python run_pipeline.py            # SAE(full) → EV → контраст(balanced) → темы
```
...
ПРОГРАММНО из Yes/No-контраста (на BALANCED-срезе) текущего SAE (+ один
случайный контроль). Явный список можно задать через `steering.neuron_ids`
в конфиге — но только если приложен «родной» SAE, породивший эти ID.

**Корректный протокол steering (Задача 3).** Прежний эксперimentъ не двигал
решение, потому что (а) alpha ±2/±5 были ~3% нормы ||h||, (б) сэмплы почти
совпадали. Исправлено:

- **alpha от нормы.** Сетка строится как доли `median||h||` на слое -5:
  `alphas = {±a·m} ∪ {0}`, `a ∈ {0.25, 0.5, 1, 2, 4}` (`steering.alpha_fractions`).
  Это на ~2 порядка крупнее прежних значений. `steering.alphas` (явный список)
  оставлен опцией — если задан, используется он.
- **Независимость сэмплов.** `temperature=1.0` и РАЗНЫЙ seed на каждый из
  `n_generations` (>= 30) сэмплов на (промпт, alpha).
- **Доверительные интервалы.** На каждую ячейку — percentile-bootstrap CI
  (1000 ресэмплов, 95%) долей Yes/No/?.
- **Отбор промптов по податливости.** Бейзлайн (alpha=0) считается заранее;
  по нему автоматически выделяются `push_down` (Yes-смещённые — есть куда
  двигать к No) и `push_up` (?/No-смещённые — есть куда к Yes), используя
  `scenario_meta` для интерпретации.
- **Критерий «сработало».** Считаем steering сработавшим, ТОЛЬКО если кривая
  триггера монотонно сдвигается по alpha **И** её CI не пересекается с CI
  контрольного нейрона на крайних alpha. Иначе — честный вывод «эффект
  неотличим от нуля». Вердикт по каждому триггеру печатается в лог; контроль
  отдельно нанесён на `steering_curves.png`.
